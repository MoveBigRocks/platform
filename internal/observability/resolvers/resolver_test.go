...
func TestIssuesRequiresActiveExtension(t *testing.T) {
	...
	ctx := graphshared.SetAuthContext(context.Background(), &platformdomain.AuthContext{
		...
		Permissions:  []string{platformdomain.PermissionIssueRead},
	})

	_, err := resolver.Issues(ctx, "ws_disabled", nil)
	if err == nil || !strings.Contains(err.Error(), "error-tracking is not active") {
