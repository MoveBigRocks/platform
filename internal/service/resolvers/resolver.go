...
type ServiceIssueProvider interface {
...
type ServiceProjectProvider interface {
...
type Config struct {
	...
	IssueService        ServiceIssueProvider
	ProjectService      ServiceProjectProvider
...
// LinkedIssues resolves the linked issues field
func (c *CaseResolver) LinkedIssues(ctx context.Context) ([]*ServiceIssueResolver, error) {
	if c.r == nil || c.r.issueService == nil || len(c.case_.LinkedIssueIDs) == 0 {
		return []*ServiceIssueResolver{}, nil
	}
	if !c.r.isErrorTrackingEnabled(ctx, c.case_.WorkspaceID) {
		return []*ServiceIssueResolver{}, nil
	}

	issues, err := c.r.issueService.GetIssuesByIDs(ctx, c.case_.LinkedIssueIDs)
	if err != nil {
		return nil, fmt.Errorf("failed to list linked issues: %w", err)
	}
	if len(issues) == 0 {
		return []*ServiceIssueResolver{}, nil
	}

	result := make([]*ServiceIssueResolver, 0, len(issues))
	for _, issue := range issues {
		if issue == nil || issue.WorkspaceID != c.case_.WorkspaceID {
			continue
		}
		result = append(result, &ServiceIssueResolver{issue: issue, r: c.r})
	}
	return result, nil
}
...
// =============================================================================
// Issue Resolver (for Case.LinkedIssues)
// =============================================================================

// ServiceIssueResolver resolves Issue in service context
type ServiceIssueResolver struct {
	issue *observabilitydomain.Issue
	r     *Resolver
}

func (i *ServiceIssueResolver) ID() model.ID        { return model.ID(i.issue.ID) }
func (i *ServiceIssueResolver) ProjectID() model.ID { return model.ID(i.issue.ProjectID) }

func (i *ServiceIssueResolver) Project(ctx context.Context) (*ServiceProjectResolver, error) {
	if i.r == nil || i.r.projectService == nil {
		return nil, nil
	}
	if !i.r.isErrorTrackingEnabled(ctx, i.issue.WorkspaceID) {
		return nil, nil
	}
	...
	return &ServiceProjectResolver{project: project, r: i.r}, nil
}

func (i *ServiceIssueResolver) Title() string { return i.issue.Title }

func (i *ServiceIssueResolver) Culprit() *string {
	if i.issue.Culprit == "" {
		return nil
	}
...
func (i *ServiceIssueResolver) Status() string { return i.issue.Status }
func (i *ServiceIssueResolver) Level() string  { return i.issue.Level }
func (i *ServiceIssueResolver) FirstSeen() graphshared.DateTime {
	return graphshared.DateTime{Time: i.issue.FirstSeen}
}
func (i *ServiceIssueResolver) LastSeen() graphshared.DateTime {
	return graphshared.DateTime{Time: i.issue.LastSeen}
}
func (i *ServiceIssueResolver) EventCount() int32 { return int32(i.issue.EventCount) }
func (i *ServiceIssueResolver) UserCount() int32  { return int32(i.issue.UserCount) }

func (i *ServiceIssueResolver) Events(ctx context.Context, args struct {
	First *int32
	After *string
}) (*ServiceErrorEventConnectionResolver, error) {
	if i.r == nil || i.r.issueService == nil {
		return &ServiceErrorEventConnectionResolver{}, nil
	}
	if !i.r.isErrorTrackingEnabled(ctx, i.issue.WorkspaceID) {
		return &ServiceErrorEventConnectionResolver{}, nil
	}
	...
	return &ServiceErrorEventConnectionResolver{events: events, limit: limit}, nil
}

func (i *ServiceIssueResolver) LinkedCase(ctx context.Context) (*CaseResolver, error) {
	if i.r == nil || i.r.caseService == nil || len(i.issue.RelatedCaseIDs) == 0 {
		return nil, nil
	}
	cases, err := i.r.caseService.GetCasesByIDs(ctx, i.issue.RelatedCaseIDs)
	if err != nil {
	...
	for _, caseObj := range cases {
		...
		if caseObj.WorkspaceID == i.issue.WorkspaceID {
			return &CaseResolver{case_: caseObj, r: i.r}, nil
		}
	}
	return nil, nil
}

func (i *ServiceIssueResolver) CreatedAt() graphshared.DateTime {
	return graphshared.DateTime{Time: i.issue.FirstSeen}
}

// ServiceProjectResolver resolves Project in service context
type ServiceProjectResolver struct {
	project *observabilitydomain.Project
	r       *Resolver
}

func (p *ServiceProjectResolver) ID() model.ID          { return model.ID(p.project.ID) }
func (p *ServiceProjectResolver) WorkspaceID() model.ID { return model.ID(p.project.WorkspaceID) }
func (p *ServiceProjectResolver) Name() string          { return p.project.Name }
func (p *ServiceProjectResolver) Slug() string          { return p.project.Slug }
func (p *ServiceProjectResolver) Platform() *string {
	if p.project.Platform == "" {
		return nil
	}
	return &p.project.Platform
}
func (p *ServiceProjectResolver) DSN() string { return p.project.DSN }

func (p *ServiceProjectResolver) Issues(ctx context.Context, args struct {
	Filter *model.IssueFilterInput
}) (*ServiceIssueConnectionResolver, error) {
	if p.r == nil || p.r.issueService == nil {
		return &ServiceIssueConnectionResolver{issues: nil, total: 0, limit: 50, r: p.r}, nil
	}
	if !p.r.isErrorTrackingEnabled(ctx, p.project.WorkspaceID) {
		return &ServiceIssueConnectionResolver{issues: nil, total: 0, limit: 50, r: p.r}, nil
	}
	limit := 50
	...
	return &ServiceIssueConnectionResolver{issues: issues, total: total, limit: limit, r: p.r}, nil
}

func (p *ServiceProjectResolver) Applications(ctx context.Context) ([]*ServiceApplicationResolver, error) {
	if p.r != nil && !p.r.isErrorTrackingEnabled(ctx, p.project.WorkspaceID) {
		return []*ServiceApplicationResolver{}, nil
	}
	return []*ServiceApplicationResolver{{app: p.project}}, nil
}

func (p *ServiceProjectResolver) CreatedAt() graphshared.DateTime {
	return graphshared.DateTime{Time: p.project.CreatedAt}
}

// ServiceIssueConnectionResolver resolves IssueConnection in service context
type ServiceIssueConnectionResolver struct {
...
func (c *ServiceIssueConnectionResolver) Edges() []*ServiceIssueEdgeResolver {
	edges := make([]*ServiceIssueEdgeResolver, len(c.issues))
	for i, issue := range c.issues {
		edges[i] = &ServiceIssueEdgeResolver{issue: issue, r: c.r}
	}
	return edges
}

func (c *ServiceIssueConnectionResolver) PageInfo() *PageInfoResolver {
	hasNext := len(c.issues) == c.limit && c.total > c.limit
	return &PageInfoResolver{hasNextPage: hasNext}
}

func (c *ServiceIssueConnectionResolver) TotalCount() int32 { return int32(c.total) }

// ServiceIssueEdgeResolver resolves IssueEdge in service context
type ServiceIssueEdgeResolver struct {
	issue *observabilitydomain.Issue
	r     *Resolver
}

func (e *ServiceIssueEdgeResolver) Node() *ServiceIssueResolver {
	return &ServiceIssueResolver{issue: e.issue, r: e.r}
}
func (e *ServiceIssueEdgeResolver) Cursor() string { return e.issue.ID }

// ServiceErrorEventConnectionResolver is a stack of issue events in service context
type ServiceErrorEventConnectionResolver struct {
	events []*observabilitydomain.ErrorEvent
	limit  int
}

func (c *ServiceErrorEventConnectionResolver) Edges() []*ServiceErrorEventEdgeResolver {
	edges := make([]*ServiceErrorEventEdgeResolver, len(c.events))
	for i, event := range c.events {
		edges[i] = &ServiceErrorEventEdgeResolver{event: event}
	}
	return edges
}

func (c *ServiceErrorEventConnectionResolver) PageInfo() *PageInfoResolver {
	hasNext := len(c.events) == c.limit
	return &PageInfoResolver{hasNextPage: hasNext}
}

func (c *ServiceErrorEventConnectionResolver) TotalCount() int32 { return int32(len(c.events)) }

// ServiceErrorEventEdgeResolver is an issue event edge in service context
type ServiceErrorEventEdgeResolver struct {
	event *observabilitydomain.ErrorEvent
}

func (e *ServiceErrorEventEdgeResolver) Node() *ServiceErrorEventResolver {
	return &ServiceErrorEventResolver{event: e.event}
}
func (e *ServiceErrorEventEdgeResolver) Cursor() string { return e.event.ID }

// ServiceErrorEventResolver resolves ErrorEvent in service context
type ServiceErrorEventResolver struct {
	event *observabilitydomain.ErrorEvent
}

func (e *ServiceErrorEventResolver) ID() model.ID      { return model.ID(e.event.ID) }
func (e *ServiceErrorEventResolver) IssueID() model.ID { return model.ID(e.event.IssueID) }
func (e *ServiceErrorEventResolver) EventID() string   { return e.event.EventID }
func (e *ServiceErrorEventResolver) Message() *string {
	if e.event.Message == "" {
		return nil
	}
	return &e.event.Message
}
func (e *ServiceErrorEventResolver) Level() string { return e.event.Level }
func (e *ServiceErrorEventResolver) Platform() *string {
	if e.event.Platform == "" {
		return nil
	}
	return &e.event.Platform
}
func (e *ServiceErrorEventResolver) Timestamp() graphshared.DateTime {
	return graphshared.DateTime{Time: e.event.Timestamp}
}
func (e *ServiceErrorEventResolver) Environment() *string {
	if e.event.Environment == "" {
		return nil
	}
	return &e.event.Environment
}
func (e *ServiceErrorEventResolver) Release() *string {
	if e.event.Release == "" {
		return nil
	}
	return &e.event.Release
}
func (e *ServiceErrorEventResolver) ExceptionType() *string {
	if len(e.event.Exception) > 0 && e.event.Exception[0].Type != "" {
		return &e.event.Exception[0].Type
	}
	return nil
}
func (e *ServiceErrorEventResolver) ExceptionValue() *string {
	if len(e.event.Exception) > 0 && e.event.Exception[0].Value != "" {
		return &e.event.Exception[0].Value
	}
	return nil
}
func (e *ServiceErrorEventResolver) Stacktrace() *ServiceStacktraceResolver {
	if len(e.event.Exception) > 0 && e.event.Exception[0].Stacktrace != nil {
		return &ServiceStacktraceResolver{stacktrace: e.event.Exception[0].Stacktrace}
	}
	if e.event.Stacktrace != nil {
		return &ServiceStacktraceResolver{stacktrace: e.event.Stacktrace}
	}
	return nil
}
func (e *ServiceErrorEventResolver) Tags() []*ServiceTagResolver {
	result := make([]*ServiceTagResolver, 0, len(e.event.Tags))
	for k, v := range e.event.Tags {
		result = append(result, &ServiceTagResolver{key: k, value: v})
	}
	return result
}
func (e *ServiceErrorEventResolver) Contexts() *graphshared.JSON {
...
func (e *ServiceErrorEventResolver) Extra() *graphshared.JSON {
...
// ServiceStacktraceResolver is a stacktrace in service context
type ServiceStacktraceResolver struct {
	stacktrace *observabilitydomain.StacktraceData
}

func (s *ServiceStacktraceResolver) Frames() []*ServiceStackFrameResolver {
	frames := make([]*ServiceStackFrameResolver, len(s.stacktrace.Frames))
	for i, frame := range s.stacktrace.Frames {
		frames[i] = &ServiceStackFrameResolver{frame: frame}
	}
	return frames
}

// ServiceStackFrameResolver is a stack frame in service context
type ServiceStackFrameResolver struct {
	frame observabilitydomain.FrameData
}

func (s *ServiceStackFrameResolver) Filename() *string {
	if s.frame.Filename == "" {
		return nil
	}
	return &s.frame.Filename
}
func (s *ServiceStackFrameResolver) Function() *string {
	if s.frame.Function == "" {
		return nil
	}
	return &s.frame.Function
}
func (s *ServiceStackFrameResolver) Module() *string {
	if s.frame.Module == "" {
		return nil
	}
	return &s.frame.Module
}
func (s *ServiceStackFrameResolver) Lineno() *int32 {
	if s.frame.LineNumber == 0 {
		return nil
	}
	line := int32(s.frame.LineNumber)
	return &line
}
func (s *ServiceStackFrameResolver) Colno() *int32 {
	if s.frame.ColNumber == 0 {
		return nil
	}
	col := int32(s.frame.ColNumber)
	return &col
}
func (s *ServiceStackFrameResolver) AbsPath() *string {
	if s.frame.AbsPath == "" {
		return nil
	}
	return &s.frame.AbsPath
}
func (s *ServiceStackFrameResolver) Context() *[]*ServiceContextLineResolver {
	if s.frame.ContextLine == "" {
		return nil
	}
	context := []*ServiceContextLineResolver{{lineNo: int32(s.frame.LineNumber), line: s.frame.ContextLine}}
	return &context
}
func (s *ServiceStackFrameResolver) InApp() *bool { return &s.frame.InApp }

// ServiceContextLineResolver is a context line in service context
type ServiceContextLineResolver struct {
	lineNo int32
	line   string
}

func (c *ServiceContextLineResolver) LineNo() int32 { return c.lineNo }
func (c *ServiceContextLineResolver) Line() string  { return c.line }

// ServiceTagResolver is a tag key/value pair in service context
type ServiceTagResolver struct {
	key   string
	value string
}

func (t *ServiceTagResolver) Key() string   { return t.key }
func (t *ServiceTagResolver) Value() string { return t.value }

// ServiceApplicationResolver is an application in service context
type ServiceApplicationResolver struct {
	app *observabilitydomain.Application
}

func (a *ServiceApplicationResolver) ID() model.ID        { return model.ID(a.app.ID) }
func (a *ServiceApplicationResolver) ProjectID() model.ID { return model.ID(a.app.ID) }
func (a *ServiceApplicationResolver) Name() string        { return a.app.Name }
func (a *ServiceApplicationResolver) Environment() *string {
	if a.app.Environment == "" {
		return nil
	}
	return &a.app.Environment
}
func (a *ServiceApplicationResolver) GitRepos() []*ServiceGitRepoResolver {
	if a.app.Repository == "" {
		return []*ServiceGitRepoResolver{}
	}
	repo := serviceGitRepoFromApplication(a.app)
	if repo == nil {
		return []*ServiceGitRepoResolver{}
	}
	return []*ServiceGitRepoResolver{repo}
}

// ServiceGitRepoResolver is a GitRepo in service context
type ServiceGitRepoResolver struct {
	repo *model.GitRepo
}

func (g *ServiceGitRepoResolver) ID() model.ID            { return g.repo.ID }
func (g *ServiceGitRepoResolver) ApplicationID() model.ID { return g.repo.ApplicationID }
func (g *ServiceGitRepoResolver) RepoURL() string         { return g.repo.RepoURL }
func (g *ServiceGitRepoResolver) DefaultBranch() string   { return g.repo.DefaultBranch }
func (g *ServiceGitRepoResolver) PathPrefix() *string     { return g.repo.PathPrefix }
func (g *ServiceGitRepoResolver) CreatedAt() graphshared.DateTime {
	return graphshared.DateTime{Time: g.repo.CreatedAt}
}

func serviceGitRepoFromApplication(app *observabilitydomain.Application) *ServiceGitRepoResolver {
...
func (r *Resolver) isErrorTrackingEnabled(ctx context.Context, workspaceID string) bool {
	if r == nil || r.extensionChecker == nil || workspaceID == "" {
		return true
	}
