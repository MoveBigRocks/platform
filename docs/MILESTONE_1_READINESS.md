...
- Repo-local blockers still remain open before Milestone 1 can honestly be called closed: ATS is still below the written owned-schema workflow scope, and the platform proof loop still does not archive live public-bundle publication metadata and digests.
...
- Reopened milestone close-out status where repo-local gaps are still real: ATS scope parity and public bundle evidence capture.
...
1. Bring ATS up to the written scope by proving extension-owned recruiting workflow state and the create, publish, review, stage-move, close, and reopen lifecycle in automated tests and proof artifacts.
