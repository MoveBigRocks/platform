...
- Repo-local blockers still remain open before Milestone 1 can honestly be called closed: ATS is still below the written owned-schema workflow scope, the `mbr` CLI still lacks sandbox create/show/extend/export/destroy flows required by scope, and the platform proof loop still does not archive live public-bundle publication metadata and digests.
...
| Hosted sandbox lifecycle and bootstrap path | `Partially evidenced` | [`internal/platform/services/sandbox_service.go`](../internal/platform/services/sandbox_service.go), [`internal/platform/services/sandbox_service_test.go`](../internal/platform/services/sandbox_service_test.go), [`internal/platform/handlers/sandbox_public.go`](../internal/platform/handlers/sandbox_public.go), [`internal/platform/handlers/runtime_bootstrap.go`](../internal/platform/handlers/runtime_bootstrap.go), [`cmd/api/routers_sandbox_test.go`](../cmd/api/routers_sandbox_test.go), [`tools/runtime-bootstrap-proof/main.go`](../tools/runtime-bootstrap-proof/main.go), [`tools/sandbox-lifecycle-proof/main.go`](../tools/sandbox-lifecycle-proof/main.go), [`scripts/run-milestone-1-proof.sh`](../scripts/run-milestone-1-proof.sh), [`docs/MILESTONE_1_PROOF.md`](./MILESTONE_1_PROOF.md) | The platform now proves create, show, export, destroy, runtime bootstrap discovery, expiry reaping, and richer lifecycle/configuration export bundles through the public API and proof artifacts, but the CLI-managed sandbox lifecycle required by scope is still missing from `mbr`. |
...
- Reopened milestone close-out status where repo-local gaps are still real: ATS scope parity, CLI sandbox lifecycle parity, and public bundle evidence capture.
...
1. Implement `mbr sandboxes create`, `show`, `extend`, `export`, and `destroy`, then fold a CLI-managed sandbox lifecycle run into the milestone proof bundle.
2. Bring ATS up to the written scope by proving extension-owned recruiting workflow state and the create, publish, review, stage-move, close, and reopen lifecycle in automated tests and proof artifacts.
