...
- Repo-local blockers still remain open before Milestone 1 can honestly be called closed: ATS is still below the written owned-schema workflow scope, sandbox expiry and richer export evidence are incomplete, and the platform proof loop still does not archive live public-bundle publication metadata and digests.
...
- Reopened milestone close-out status where repo-local gaps are still real: ATS scope parity, sandbox expiry and export, and public bundle evidence capture.
...
1. Close the sandbox lifecycle gaps by implementing auto-expiry or a reaper flow, enriching export output beyond handoff-only metadata, and archiving that evidence as part of the milestone proof bundle.
