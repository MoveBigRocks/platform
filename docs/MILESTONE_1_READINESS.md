...
Those runs emit `*.publication-evidence.json` artifacts that are now fetched
from the checked-in run manifest and archived in the milestone proof bundle
under `public-bundle-publication/release-evidence/` by the GitHub proof
workflow. Local reruns can use the same manifest or provide a
pre-downloaded evidence directory directly.
