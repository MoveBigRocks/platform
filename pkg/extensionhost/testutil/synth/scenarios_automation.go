...
func (sr *AutomationScenarioRunner) RunRuleConditionEvaluationScenario(ctx context.Context, workspaceID string, agents []*platformdomain.User) (*ScenarioResult, error) {
	...
	if err := sr.services.Store.Rules().CreateRule(ctx, rule); err != nil {
		return result, fmt.Errorf("create high priority rule: %w", err)
	}
	...
	if err := sr.services.Store.Contacts().CreateContact(ctx, contact); err != nil {
		return result, fmt.Errorf("create condition test contact: %w", err)
	}
	...
	if err := sr.services.Store.Cases().CreateCase(ctx, highCase); err != nil {
		return result, fmt.Errorf("create high priority case: %w", err)
	}
	...
	if err := sr.services.Store.Cases().CreateCase(ctx, lowCase); err != nil {
		return result, fmt.Errorf("create low priority case: %w", err)
	}
...
func (sr *AutomationScenarioRunner) RunRuleActionExecutionScenario(ctx context.Context, workspaceID string, agents []*platformdomain.User) (*ScenarioResult, error) {
	...
	if err := sr.services.Store.Contacts().CreateContact(ctx, contact); err != nil {
		return result, fmt.Errorf("create action test contact: %w", err)
	}
	...
	if err := sr.services.Store.Cases().CreateCase(ctx, testCase); err != nil {
		return result, fmt.Errorf("create action test case: %w", err)
	}
	...
	if err := sr.services.Store.Cases().UpdateCase(ctx, testCase); err != nil {
		return result, fmt.Errorf("apply assign action: %w", err)
	}
	...
	if err := sr.services.Store.Cases().UpdateCase(ctx, testCase); err != nil {
		return result, fmt.Errorf("apply add-tags action: %w", err)
	}
	...
	if err := sr.services.Store.Cases().UpdateCase(ctx, testCase); err != nil {
		return result, fmt.Errorf("apply change-status action: %w", err)
	}
	...
	if err := sr.services.Store.Cases().UpdateCase(ctx, testCase); err != nil {
		return result, fmt.Errorf("apply change-priority action: %w", err)
	}
	...
	if err := sr.services.Store.Rules().CreateRule(ctx, rule); err != nil {
		return result, fmt.Errorf("create action test rule: %w", err)
	}
...
func (sr *AutomationScenarioRunner) RunJobQueueScenario(ctx context.Context, workspaceID string, agents []*platformdomain.User) (*ScenarioResult, error) {
	...
	job.Attempts = 1
	if err := sr.services.Store.Jobs().UpdateJob(ctx, job); err != nil {
	...
	if err := job.MarkCompleted(map[string]any{
		"message_id": "msg-123",
		"delivered":  true,
	}); err != nil {
	...
	if err := sr.services.Store.Jobs().UpdateJob(ctx, job); err != nil {
		return result, fmt.Errorf("persist completed job: %w", err)
	}
...
func (sr *AutomationScenarioRunner) RunScheduledJobScenario(ctx context.Context, workspaceID string, agents []*platformdomain.User) (*ScenarioResult, error) {
	...
	if err := sr.services.Store.Jobs().CreateJob(ctx, immediateJob); err != nil {
		return result, fmt.Errorf("create immediate job: %w", err)
	}
	...
	if err := sr.services.Store.Jobs().CreateJob(ctx, failedJob); err != nil {
		return result, fmt.Errorf("create failed job: %w", err)
	}
	...
	if err := sr.services.Store.Jobs().UpdateJob(ctx, failedJob); err != nil {
		return result, fmt.Errorf("persist failed job state: %w", err)
	}
	...
	if err := sr.services.Store.Jobs().UpdateJob(ctx, failedJob); err != nil {
		return result, fmt.Errorf("persist retrying job state: %w", err)
	}
