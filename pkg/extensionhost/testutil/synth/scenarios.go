...
func (sr *ScenarioRunner) RunFullConversationScenario(ctx context.Context, workspaceID string, agents []*platformdomain.User) (*ScenarioResult, error) {
	...
	// Initial email
	if err := sr.createInboundEmail(ctx, storedCase, contact, storedCase.Description); err != nil {
		return result, fmt.Errorf("failed to create initial inbound email: %w", err)
	}

	// Exchange 1: Agent asks clarifying question
	if err := sr.createOutboundEmail(ctx, storedCase, agent, contact, "Could you please confirm which billing cycle this relates to?"); err != nil {
		return result, fmt.Errorf("failed to create clarification email: %w", err)
	}
	result.TotalMessages++
	...
	storedCase.Status = servicedomain.CaseStatusPending
	if err := sr.services.Store.Cases().UpdateCase(ctx, storedCase); err != nil {
		return result, fmt.Errorf("failed to move case to pending: %w", err)
	}
	result.StateTransitions = append(result.StateTransitions, StateTransition{
	...
	// Customer clarifies
	if err := sr.createInboundEmail(ctx, storedCase, contact, "It's the December invoice dated December 1st."); err != nil {
		return result, fmt.Errorf("failed to record customer clarification: %w", err)
	}
	result.TotalMessages++
	...
	storedCase.Status = servicedomain.CaseStatusOpen
	if err := sr.services.Store.Cases().UpdateCase(ctx, storedCase); err != nil {
		return result, fmt.Errorf("failed to reopen case after clarification: %w", err)
	}
	result.StateTransitions = append(result.StateTransitions, StateTransition{
		FromStatus: oldStatus, ToStatus: storedCase.Status, Timestamp: time.Now(),
	...
	if err := sr.createOutboundEmail(ctx, storedCase, agent, contact, "I found the issue - there was a prorated charge. Would you like a refund?"); err != nil {
		return result, fmt.Errorf("failed to send resolution email: %w", err)
	}
	result.TotalMessages++
	...
	storedCase.Status = servicedomain.CaseStatusPending
	if err := sr.services.Store.Cases().UpdateCase(ctx, storedCase); err != nil {
		return result, fmt.Errorf("failed to move case back to pending: %w", err)
	}
	result.StateTransitions = append(result.StateTransitions, StateTransition{
		FromStatus: oldStatus, ToStatus: storedCase.Status, Timestamp: time.Now(),
		TriggerType: "agent_reply", ActorID: agent.ID, ActorName: agent.Name,
	...
	// Customer confirms
	if err := sr.createInboundEmail(ctx, storedCase, contact, "Yes please, remove the charge and refund."); err != nil {
		return result, fmt.Errorf("failed to record refund confirmation: %w", err)
	}
	result.TotalMessages++
	...
	storedCase.Status = servicedomain.CaseStatusOpen
	if err := sr.services.Store.Cases().UpdateCase(ctx, storedCase); err != nil {
		return result, fmt.Errorf("failed to reopen case for final action: %w", err)
	}
	result.StateTransitions = append(result.StateTransitions, StateTransition{
		FromStatus: oldStatus, ToStatus: storedCase.Status, Timestamp: time.Now(),
		TriggerType: "customer_email", ActorID: contact.ID, ActorName: contact.Name,
	...
	if err := sr.createOutboundEmail(ctx, storedCase, agent, contact, "Done! Refund processed. Is there anything else?"); err != nil {
		return result, fmt.Errorf("failed to send final email: %w", err)
	}
	result.TotalMessages++
	...
	storedCase.ClosedAt = &now
	if err := sr.services.Store.Cases().UpdateCase(ctx, storedCase); err != nil {
		return result, fmt.Errorf("failed to close case: %w", err)
	}
	result.StateTransitions = append(result.StateTransitions, StateTransition{
...
func (sr *ScenarioRunner) RunEscalationScenario(ctx context.Context, workspaceID string, agents []*platformdomain.User) (*ScenarioResult, error) {
	...
	if err := sr.createInboundEmail(ctx, storedCase, contact, storedCase.Description); err != nil {
		return result, fmt.Errorf("failed to record inbound escalation email: %w", err)
	}

	// Tier 1 picks up
	storedCase.AssignedToID = tier1Agent.ID
	if err := sr.services.Store.Cases().UpdateCase(ctx, storedCase); err != nil {
		return result, fmt.Errorf("failed to assign tier 1 agent: %w", err)
	}

	if err := sr.createOutboundEmail(ctx, storedCase, tier1Agent, contact, "I'm escalating this to our senior team immediately."); err != nil {
		return result, fmt.Errorf("failed to send escalation email: %w", err)
	}
	result.TotalMessages++
	...
	if err := sr.services.Store.Cases().UpdateCase(ctx, storedCase); err != nil {
		return result, fmt.Errorf("failed to escalate case: %w", err)
	}
	result.StateTransitions = append(result.StateTransitions, StateTransition{
		FromStatus: oldStatus, ToStatus: storedCase.Status, Timestamp: time.Now(),
	...
	// Tier 2 resolves
	if err := sr.createOutboundEmail(ctx, storedCase, tier2Agent, contact, "I've identified and fixed the issue. Can you confirm it's working?"); err != nil {
		return result, fmt.Errorf("failed to send tier 2 update: %w", err)
	}
	result.TotalMessages++

	if err := sr.createInboundEmail(ctx, storedCase, contact, "Yes! It's back! Thank you!"); err != nil {
		return result, fmt.Errorf("failed to record recovery confirmation: %w", err)
	}
	result.TotalMessages++
	...
	storedCase.ClosedAt = &now
	if err := sr.services.Store.Cases().UpdateCase(ctx, storedCase); err != nil {
		return result, fmt.Errorf("failed to resolve escalated case: %w", err)
	}
	result.StateTransitions = append(result.StateTransitions, StateTransition{
...
func (sr *ScenarioRunner) RunAutoCloseScenario(ctx context.Context, workspaceID string, agents []*platformdomain.User) (*ScenarioResult, error) {
	...
	storedCase, _ := sr.services.Store.Cases().GetCase(ctx, caseObj.ID)
	if err := sr.createInboundEmail(ctx, storedCase, contact, storedCase.Description); err != nil {
		return result, fmt.Errorf("failed to record pricing question: %w", err)
	}

	// Agent responds
	storedCase.AssignedToID = agent.ID
	if err := sr.services.Store.Cases().UpdateCase(ctx, storedCase); err != nil {
		return result, fmt.Errorf("failed to assign pricing case: %w", err)
	}

	if err := sr.createOutboundEmail(ctx, storedCase, agent, contact, "Here's the comparison: Pro is $99/mo, Enterprise is $299/mo with unlimited users..."); err != nil {
		return result, fmt.Errorf("failed to send pricing response: %w", err)
	}
	result.TotalMessages++

	storedCase.Status = servicedomain.CaseStatusPending
	if err := sr.services.Store.Cases().UpdateCase(ctx, storedCase); err != nil {
		return result, fmt.Errorf("failed to move pricing case to pending: %w", err)
	}
	result.StateTransitions = append(result.StateTransitions, StateTransition{
	...
	if err := sr.createOutboundEmail(ctx, storedCase, agent, contact, "Hi Dave, just checking in - any questions about pricing?"); err != nil {
		return result, fmt.Errorf("failed to send follow-up email: %w", err)
	}
	result.TotalMessages++
	...
	if err := sr.services.Store.Cases().UpdateCase(ctx, storedCase); err != nil {
		return result, fmt.Errorf("failed to auto-close pricing case: %w", err)
	}
	result.StateTransitions = append(result.StateTransitions, StateTransition{
...
func (sr *ScenarioRunner) RunReopenScenario(ctx context.Context, workspaceID string, agents []*platformdomain.User) (*ScenarioResult, error) {
	...
	storedCase, _ := sr.services.Store.Cases().GetCase(ctx, caseObj.ID)
	if err := sr.createInboundEmail(ctx, storedCase, contact, storedCase.Description); err != nil {
		return result, fmt.Errorf("failed to record initial reopen email: %w", err)
	}

	storedCase.AssignedToID = agent.ID
	if err := sr.services.Store.Cases().UpdateCase(ctx, storedCase); err != nil {
		return result, fmt.Errorf("failed to assign reopen case: %w", err)
	}

	if err := sr.createOutboundEmail(ctx, storedCase, agent, contact, "Try clearing your cache and trying again."); err != nil {
		return result, fmt.Errorf("failed to send first fix email: %w", err)
	}
	result.TotalMessages++

	if err := sr.createInboundEmail(ctx, storedCase, contact, "That worked, thanks!"); err != nil {
		return result, fmt.Errorf("failed to record first confirmation: %w", err)
	}
	result.TotalMessages++
	...
	storedCase.ClosedAt = &now
	if err := sr.services.Store.Cases().UpdateCase(ctx, storedCase); err != nil {
		return result, fmt.Errorf("failed to resolve initial reopen case: %w", err)
	}
	result.StateTransitions = append(result.StateTransitions, StateTransition{
	...
	if err := sr.createInboundEmail(ctx, storedCase, contact, "Actually, the issue is back with a different error."); err != nil {
		return result, fmt.Errorf("failed to record reopen message: %w", err)
	}
	result.TotalMessages++
	...
	if err := sr.services.Store.Cases().UpdateCase(ctx, storedCase); err != nil {
		return result, fmt.Errorf("failed to reopen case state: %w", err)
	}
	result.StateTransitions = append(result.StateTransitions, StateTransition{
		FromStatus: oldStatus, ToStatus: servicedomain.CaseStatusOpen,
		Timestamp: time.Now(), TriggerType: "customer_email",
	...
	if err := sr.createOutboundEmail(ctx, storedCase, agent, contact, "I found a new bug we just fixed. Please try now."); err != nil {
		return result, fmt.Errorf("failed to send second fix email: %w", err)
	}
	result.TotalMessages++

	if err := sr.createInboundEmail(ctx, storedCase, contact, "Working now! Thanks!"); err != nil {
		return result, fmt.Errorf("failed to record final confirmation: %w", err)
	}
	result.TotalMessages++
	...
	storedCase.ClosedAt = &now
	if err := sr.services.Store.Cases().UpdateCase(ctx, storedCase); err != nil {
		return result, fmt.Errorf("failed to close reopened case: %w", err)
	}
	result.StateTransitions = append(result.StateTransitions, StateTransition{
