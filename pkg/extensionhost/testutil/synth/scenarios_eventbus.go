...
func (sr *EventBusScenarioRunner) scenarioFileEventBusPublishSubscribe(ctx context.Context) (*ScenarioResult, error) {
	...
	go func() {
		if subErr := eventBus.Subscribe(eventbus.StreamCaseEvents, "test-group", "test-consumer", func(ctx context.Context, data []byte) error {
			receivedMu.Lock()
			receivedEvent = data
			receivedMu.Unlock()
			select {
			...
			sr.log("subscriber stopped with error: %v", subErr)
		}
	}()
	...
	if err := eventBus.Publish(eventbus.StreamCaseEvents, testEvent2); err != nil {
		result.Error = err
		result.Verifications = append(result.Verifications, VerificationResult{
			Check: "Follow-up event published", Passed: false, Details: fmt.Sprintf("Error: %v", err),
		})
		return result, err
	}
...
func (sr *EventBusScenarioRunner) scenarioOutboxReliableDelivery(ctx context.Context) (*ScenarioResult, error) {
	...
	outboxService.Start()
	defer func() {
		if stopErr := outboxService.Stop(5 * time.Second); stopErr != nil {
			sr.log("outbox stop error: %v", stopErr)
		}
	}()
...
func (sr *EventBusScenarioRunner) scenarioMultipleSubscribers(ctx context.Context) (*ScenarioResult, error) {
	...
	go func() {
		if err := eventBus.Subscribe(eventbus.StreamJobEvents, "group1", "consumer1", func(ctx context.Context, data []byte) error {
			atomic.AddInt32(&received1, 1)
			return nil
		}); err != nil {
			sr.log("subscriber group1 stopped: %v", err)
		}
	}()

	go func() {
		if err := eventBus.Subscribe(eventbus.StreamJobEvents, "group2", "consumer2", func(ctx context.Context, data []byte) error {
			atomic.AddInt32(&received2, 1)
			return nil
		}); err != nil {
			sr.log("subscriber group2 stopped: %v", err)
		}
	}()

	go func() {
		if err := eventBus.Subscribe(eventbus.StreamJobEvents, "group3", "consumer3", func(ctx context.Context, data []byte) error {
			atomic.AddInt32(&received3, 1)
			return nil
		}); err != nil {
			sr.log("subscriber group3 stopped: %v", err)
		}
	}()
	...
	for i := 0; i < numEvents; i++ {
		...
		if err := eventBus.Publish(eventbus.StreamJobEvents, event); err != nil {
			result.Error = err
			result.Verifications = append(result.Verifications, VerificationResult{
				Check: "Events published", Passed: false, Details: fmt.Sprintf("Error: %v", err),
			})
			return result, err
		}
	}
...
func (sr *EventBusScenarioRunner) scenarioEndToEndEventFlow(ctx context.Context) (*ScenarioResult, error) {
	...
	go func() {
		if err := eventBus.Subscribe(eventbus.StreamCaseCommands, "case-handlers", "handler-1", func(ctx context.Context, data []byte) error {
			...
			if err := json.Unmarshal(data, &event); err == nil {
				...
				if err := eventBus.Publish(eventbus.StreamCaseEvents, responseEvent); err != nil {
					return err
				}
			}
			...
			sr.log("case command subscriber stopped: %v", err)
		}
	}()
